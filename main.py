"""
Amodal Completion Pipeline — minimal InstaFormer + SAM3 + Flux-Fill stack
==========================================================================

The entry point is test_flux_cutout_person.py, which calls `occlusion_agent`
(Agent 1, below) directly and then `_run_flux_fill_inpaint`. There is no
LangGraph, no reviewer/retry agent, and no CLI entry point in this module —
the older multi-agent graph (inpainting_agent / reviewer / route / build_graph
/ run) was removed once it was confirmed nothing still called it.
...
Agent 1 — Occlusion Agent
    ...
    Occluder-mask candidates are fused (see `_fuse_masks` /
...
`_run_flux_fill_inpaint`
    FLUX.1-Fill-dev inpaints the hidden region (dilate(occluder) \\ visible,
    Jiang-Ao-style) directly — no shape-prior model, no LaMa background fill.
    Falls back to `_run_controlnet_inpaint` only if the Flux pipeline itself
    fails to load.

Removed (permanently disabled in config.py, superseded by the stack above —
see git log for details): pix2gestalt, AISFormer-head inference, LaMa,
ControlNet+SD-1.5 as a primary backend, CLIP-grounded segmentation,
GroundingDINO, PSALM, Mixed Context Diffusion Sampling (mixed_context.py).
ControlNet+SD-1.5 code remains ONLY as `_run_flux_fill_inpaint`'s
...
def _free_all_gpu_models_except(*keep_names: str):
    ...
    `keep_names` (one of: 'sam3', 'sam3_text', 'flux', 'controlnet').
...
# ControlNet+SD-1.5 is NOT a live inpainting path any more (INPAINT_BACKEND
# is always "flux_fill"), but `_run_flux_fill_inpaint` falls back to it if
# FluxFillPipeline fails to load (see `_get_flux_fill_pipe` returning None),
# so this stays as the pipeline's only inpainting fallback. Keep it.
...
# ── ControlNet-Inpaint (kept only as _run_flux_fill_inpaint's load-failure ───
# fallback — see note on _run_controlnet_inpaint below) ──────────────────────

def _prep_control_image(
    base_np: np.ndarray,        # H×W×3 uint8 RGB (expanded canvas)
...
def _run_controlnet_inpaint(
    base_np: np.ndarray,         # H×W×3 uint8 RGB scene
    ...
    original_image_np: Optional[np.ndarray] = None,  # unused; kept for call-site compatibility
    visible_mask: Optional[np.ndarray] = None,        # unused; kept for call-site compatibility
    seed_offset: int = 0,        # per-attempt offset so retries produce different samples
    frame_cropped: bool = False, # unused; kept for call-site compatibility
) -> list:
    """
    ...
    NOTE: this is no longer a live inpainting path on its own — config.
    INPAINT_BACKEND is always "flux_fill", so the dispatch below always
    returns `_run_flux_fill_inpaint(...)`.  The ControlNet+SD-1.5 code past
    that dispatch is kept only because `_run_flux_fill_inpaint` falls back
    to it if FluxFillPipeline fails to load (see `_get_flux_fill_pipe`).

    Mixed Context Diffusion Sampling (Xu et al., CVPR 2024) used to run in
    this function when config.USE_MIXED_CONTEXT was True — permanently
    disabled and removed along with LaMa and mixed_context.py.
    """
    ...
    # else fall through to the ControlNet-Inpaint path below.
    from PIL import Image as PILImage
    ...
    pil_base  = Image.fromarray(base_np)
    pil_mask  = Image.fromarray((inpaint_mask * 255).astype(np.uint8)).convert("L")
    ctrl_img  = _prep_control_image(base_np, inpaint_mask, amodal_rgb_256, orig_hw)
